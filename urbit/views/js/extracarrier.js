...
var ExtraCarrier = function(options)
{
	...
	this.renderMoreInformation = function()
	{
		$(ExtraCarrier.instance._classNameCarrier).each(function()
		{
			...
			if (idCarrier <= 0)
				return;
			// Showing delivery time
			if (ExtraCarrier.instance._urbitShowDelay === 1 && typeof ExtraCarrier.instance._urbitDelays[idCarrier] !== 'undefined')
				$(this).parent(ExtraCarrier.instance._classNameParent).find(ExtraCarrier.instance._classNameDelays).html(ExtraCarrier.instance._urbitDelays[idCarrier]);
			// Not showing delivery time
			if (ExtraCarrier.instance._urbitShowDelay === 0)
				$(this).parent(ExtraCarrier.instance._classNameParent).find(ExtraCarrier.instance._classNameDelays).html('');
			// Showing partly costs
			if (ExtraCarrier.instance._urbitShowPartlyCost === 1 && typeof ExtraCarrier.instance._urbitPartlyCosts[idCarrier] !== 'undefined')
			{
				$(this).parent(ExtraCarrier.instance._classNameParent).find(ExtraCarrier.instance._classNamePartlyCosts).html('');
				$(this).parent(ExtraCarrier.instance._classNameParent).find(ExtraCarrier.instance._classPrice).append(ExtraCarrier.instance._urbitPartlyCosts[idCarrier]);
			}
		});
	};
};
